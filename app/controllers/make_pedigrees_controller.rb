class MakePedigreesController < BaseController
  def index
    ...
    pacientes.each do |patient|
      ...
      pat = Patient.create!(name: patient['Nombre'], lastname: patient['Apellido'], birth_date: DateTime.strptime(patient['Fecha_Nac'], '%Y-%m-%d %H:%M:%S'), gender: patient['Sexo'], pedigree: pedigree, active: true, status: 'alive', patient_type: 'patient')
